...
<head>
    <meta charset="UTF-8">
    <title>permissionManagePage</title>
</head>
</br>
<div align="center">
    回到首页：
...
<body>
<div align="center">
    <form action="${PageContext.request.getContextPath}/testboot/addpermission" method="POST">
        <table border="1" style="background-color: dodgerblue">
            <tr>
<%--                <td><input id="c" type="checkbox" name="selectall" onclick="selectAll()"></td>--%>
            </tr>
            <tr>
                <td><input type="checkbox" name="u_id" onclick="toCheck()" value="${user.id}">
                    <c:if test="$ind.index==1">checked="checked"</c:if></td>
                <td>${user.id}</td>
                <td>${user.u_name}</td>
                <td>${user.username}</td>
                <c:forEach items="${addablePer}" var="pList" varStatus="ind1">
                    <td><input type="checkbox" name="perid" onclick="toCheck()" value="${pList.id}">
                        <c:if test="$ind1.index==1">checked="checked"</c:if></td>
                    <td>${pList.r_name}</td>
                </c:forEach>
            </tr>
        </table>
